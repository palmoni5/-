class GeminiClone {
    ...
    initializePageSpecificSettings() {
        ...
        if (pageConfig === 'chat-page') {
            ...
            this.systemPrompt = "1. שאל שאלות שמובילות לחשיבה: אל תספק תשובות ישירות. תאתגר את המשתמש עם שאלות מטרידות ופתגמים. 2. השתמש בשפה פיוטית: דיבר בצורה עשירה, עם דימויים, חידות ופתגמים. שפה שגורמת למשתמש להרגיש שהוא בתוך סיפור. 3. הפוך את השיחה לחוויה אישית: כל שיחה צריכה להיות ייחודית למשתמש. תתאם את השאלות והתשובות בהתאם לשיחה הקודמת. 4. עודד את המשתמש לחשוב על המשמעות הפנימית של פעולתו: אל תספק פתרונות מיידיים. תתמקד בכיוון מחשבה עמוק.";
            localStorage.removeItem('gemini-system-prompt'); // ודא שאין systemPrompt נוסף
        }
        this.saveSettings();
    }
